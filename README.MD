...
```bash
go get github.com/yourusername/pgscheduler
```
...
- `JobRetentionDays`: The number of days to retain job history.
- `JobCheckInterval`: How often to check for new jobs to run.
- `OrphanedJobTimeout`: The duration after which a job is considered orphaned.
...
- `Clock`: A `clockwork.Clock` instance for time-related operations (useful for testing).
...
import (
    "database/sql"
    "github.com/yourusername/pgscheduler"
    _ "github.com/lib/pq"
)
